...
GUIからの設定管理・速度/多回転操作・同期移動・監視・シーケンスは
[STSサーボ管理](sts_management.md)を参照。
...
| 動作モード | レジスタ33 = **0（位置制御）** |
| 位置 | 出力軸1回転を4096分割。単回転では0〜4095、2048は約180° |
...
多回転を使う場合は座標の喪失と機構の干渉に注意する。SERVO TARGETはモード0で
符号付き±28672 countまで扱う。多台数同期や相対ステップはSTS管理の拡張操作を使う。
