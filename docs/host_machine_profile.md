...
`speed_per_second`とモータ側保護上限を機体単位へ換算した値のうち、小さい方を実効最高速度とする。
スティック値×`input_sign`×実効最高速度が機体単位の速度になる。
...
実効最高速度に対する割合で指定する。範囲は1〜100%、省略時は20%。
実効速度は`実効最高速度 × homing_speed_percent ÷ 100`となる。
