...
`0=LX, 1=LY, 2=RX, 3=RY, 4=L2, 5=R2, 6=R2-L2`。6はトリガーを
一つの符号付き入力として扱い、R2が正、L2が負、同時押しは差し引く。
省略した軸の速度入力は0になる。現在の機体設定はLX=θ、LY=r、R2-L2=zで、
RXはEE全体回転に使用する。
...
原点は手動採用または自動ホーミングで採用する。「原点位置まで手動で移動する」を有効にして運転中、
リミットへ未到達から到達へ変化した軸は、その場で`origin_position`を自動採用する。通常運転中や、
調整開始時から接点が作動している場合は自動採用しない。到達中は近づく方向だけ止める。接点情報が欠けている
...
PWMサーボはchannel 0..3、パルス幅100..20000µs。PWM基板の受理範囲は、設定した全PWMサーボの
移動下限・上限を包む値へhostが自動設定する。STS3215はID 1..253、通常の設定値は位置0..4095、
