...
4. **3. 読取り** で状態・ENC1・接点/DIP・受信ログを必要に応じてONにする。
5. **4. 停止** の「全出力STOP」で停止、「停止してテストを終了」でポートを通常操作へ戻す。

値を変えた後は「送り直す」で目標と自動OFF期限を更新する。出力中の行には
自動OFFまでの残り秒数が出る。serial_svmdは「IDを追加」で最大16行まで操作できる。テスト中は通常のゲームパッド送信・RUN・生コマンドを禁止する。
通常操作画面やSpaceのSTOPもテスト側に届く。キー操作は他の画面と同じで、
入力欄を編集している間だけキーが入力になる（Escでフォーカスを外す）。
タブを切り替えてもテストは継続する。
終了後の通常送信は自動再開しない。「3 操作」画面で
「コントローラ入力の送信を有効化」を入れ直す。入れないままRUNすると、目標指令が
流れずWatchdogでSTOPへ落ちる。通信エラー後もテスト終了を明示するまで通常操作を禁止する。

テストへの切替時は通常操作で使用していた基板にもSTOPを送る。
テスト中の接続設定は固定し、変更するには一度テストを終了する。
GUIとCLIは同じ制御・通信処理を使用する。以下の接続、安全上の注意、単位は両方に共通する。
