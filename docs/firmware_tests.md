...
起動済みhostでの機体操作・速度テストは[host操作ガイド](host_operation.md)のAPIを使う。
この文書の`fw_test`はhostを終了して使う独立した基板診断ツール。
...
host GUIでは「診断」の配線ガイドで同じ内容を見られる。
...
通知の時刻や実際の停止タイミングも確認する。serial_svmdはCAN経由ではstatus=2を通知するが、
USART2のASCII経由では自発的なタイムアウト通知がない。
