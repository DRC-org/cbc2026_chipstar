...
SerialSVMDへの指令はCAN RX0割込みで32フレームのキューに退避し、メインループで
順に実行する。サーボの読戻しやトルク解除中も受信を継続するため、単体テスト開始時の
STOP・SAFE・TARGET・ENABLE・RUNが深さ3のハードウェアFIFOから溢れるのを防ぐ。
FIFOまたはキューの欠落、通信期限を超えて滞留した指令を検出した場合は、残りの指令を
破棄してSTOPに移る。通信期限は処理時刻ではなく受信時刻から数える。
一括監視の分割データは2ms間隔で送信する。CCTLはFDCAN2受信割込みで64フレームの
キューに退避し、USB送信バッファに空きがあるときにhostへ転送する。
受信FIFOまたはキューの欠落は`CAN_RX_LOST bus=2 count=...`で報告する。
