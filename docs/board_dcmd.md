...
hostがチャネルと符号付きDutyを指定する。出力上限・加減速・反転待ちは実行時パラメータで変更できる。
FW起動時の初期値は上限±900 permille（±90%）、10msごとに1 permilleずつの出力変化、
反転待ち2000ms。機体hostは移動出力を基板の上限へ自動反映する。
方向反転時はゼロまで減速し、設定した時間だけブレーキを維持してから逆方向へ立ち上げる。
STOP/SAFE/通信断はランプを待たずゼロにし、その時点から反転待ちを数える。
停止中に経過した時間も含め、停止指令の再送や逆転開始で待ち直さない。
RUNの再送もDutyランプの更新時刻を変更しない。
