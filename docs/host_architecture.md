...
速度・電流・温度などのデバイス保護、SAFE/STOP、通信監視を担当する。機体固有の動作をFWへ持ち込まず、
...
単体テストは原点・リミット・速度換算、基板ごとの応答照合、保守ツールを検証する。
`host/tests/simulated_host.rs`は実行ファイルを起動し、実際のソケットで操作権、入力期限、
停止・保持と全出力停止、RUN応答待ち中の停止、通信断と再接続、異常後の復旧、
運転中に拒否する設定変更、一時適用と保存を検証する。さらにSTATEテレメトリへ動作モード、
有効軸、stale、異常bit、再起動、位置飛びを注入し、実際の受信経路から停止と原点喪失を確認する。
模擬接続は衝突や実際の制動距離を再現しないため、機構の動作確認は
[立ち上げ手順](bringup.md)に従って実機で行う。

`gui::workflow_tests`はGUI、共有要求キュー、実ワーカー、模擬機体を接続し、人間だけが行える
緊停解除と、画面・診断項目の切替を検証する。通常運転は画面移動で継続し、個別テストと
STS3215出力は解除されること、各出力中の緊停と解除で自動再開しないことを確認する。
...
通信は通常操縦と同じワーカーの`Link`を使用する。通常操縦の停止・保持は画面遷移で解除しない。
個別テストとSTS3215の出力は、診断項目または画面を移動すると停止・解除する。
速度・DC出力はGUIの押下更新が150ms途切れた場合にもワーカーが解除する。
